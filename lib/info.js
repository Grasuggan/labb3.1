...
import { ALL_EMOTIONS_QUERY } from "../pages/courses/[course]";
...
export async function getAllEmotions() {
  ...
  const allEmotionsData = await apolloClient.query({
    query: ALL_EMOTIONS_QUERY,
  });

  return allEmotionsData.data.queryEmotion.map((c) => {
    return {
      params: {
        status: c.title,
      },
